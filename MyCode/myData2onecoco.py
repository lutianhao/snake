...
if __name__ == '__main__':                            #设置相关路径
    ...
    Json_save_PATH = '../data/NICE1/NICE1/coco/train/annotations/NICE_train.json' #json输出路径
    ...
    # org_ini_folder = os.path.join(Param_bbox_PATH,'test/iris_edge_bbox/')
    ...
    for imgpath in imglist:
        ...
        if not os.path.isfile(org_ini_folder+ininame+'.ini'):
            ...
            ann = {'area': w*h, 'iscrowd': 0, 'image_id':
                   image_id, 'bbox': bbox,
                   'category_id': 1, 'id': int(len(ret['annotations']) + 1), 'ignore': 0,
                   'segmentation': []
