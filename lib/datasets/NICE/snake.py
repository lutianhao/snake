...
class Dataset(data.Dataset):
    ...
    def transform_original_data(self, instance_polys, flipped, width, trans_output, inp_out_hw):  #变换原始信息
    ...
    def get_valid_polys(self, instance_polys, inp_out_hw):   #得到有效的多边形
        output_h, output_w = inp_out_hw[2:]
        instance_polys_ = []
        for instance in instance_polys:
    ...
    def get_extreme_points(self, instance_polys):    #得到极值点（比如目标最左边最右最上最下点的横纵坐标）
        extreme_points = []
        for instance in instance_polys:
    ...
    def prepare_detection(self, box, poly, ct_hm, cls_id, wh, reg, ct_cls, ct_ind):  #准备原来尺寸的bbox吧？
        ...
        radius = data_utils.gaussian_radius((math.ceil(h), math.ceil(w))) #math.ceil是向上取整
    ...
    def prepare_init(self, box, extreme_point, i_it_4pys, c_it_4pys, i_gt_4pys, c_gt_4pys, h, w):  #准备初始化？
    ...
    def prepare_evolution(self, poly, extreme_point, img_init_polys, can_init_polys, img_gt_polys, can_gt_polys): #准备进化？
        x_min, y_min = np.min(extreme_point[:, 0]), np.min(extreme_point[:, 1])
        x_max, y_max = np.max(extreme_point[:, 0]), np.max(extreme_point[:, 1])
    ...
    def __getitem__(self, index):
        ...
        for key in anno:
            tmp_file.writelines("anno key:" + str(key) + "\n")
        ...
        tmp_file.writelines("img type:" + str(type(img)) + "\n")
        tmp_file.writelines("img len:" + str(img.shape) + "\n")
